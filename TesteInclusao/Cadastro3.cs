...
namespace TesteInclusao
{
	public class Cadastro3
	{
		public static int CadastrarPessoa(Pessoa pessoa, OleDbConnection conexaodb)
		{
			try
			{
				string query = $"INSERT INTO PESSOA(NOME, CPF, IDENDERECO) VALUES ('{pessoa.Nome}', '{pessoa.Cpf}', {pessoa.Endereco.Id})";
		...
		public static int CadastrarTelefone(Telefone telefone, int idPessoa, OleDbConnection conexaodb)
		{
			try
			{
				string query = $"INSERT INTO TELEFONE(IDPESSOA, NUMEROTELEFONE, DDD, TIPO) VALUES ({idPessoa},{telefone.NumeroTelefone}, {telefone.Ddd}, '{telefone.Tipo}')";
				OleDbCommand cmd = new OleDbCommand(query, conexaodb);
				cmd.ExecuteNonQuery();
